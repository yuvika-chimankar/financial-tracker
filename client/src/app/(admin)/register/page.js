...
const Register = () => {
  ...
  const handleRegistration = async (e) => {
    ...
    if (response.ok) {
      ...
      localStorage.setItem('user_name', data.user[1]);
      localStorage.setItem('user_email', data.user[2]);
      router.push('/home');
    } else {
      alert(data.message);
    }
    ...
    <>
      <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-sm">
          <h1 className="mt-10 text-center text-2xl/9 font-bold tracking-tight text-gray-900">Finlytics</h1>
        ...
        <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
          ...
          <p className="mt-10 text-center text-sm/6 text-gray-500">
            ...
            <span onClick={() => router.push('/login')} className="font-semibold text-indigo-600 hover:text-indigo-500 cursor-pointer">Login</span>
          </p>
        </div>
      </div>
    </>
